...
class FlightsInput(BaseModel):
    ...
    adults: int = Field(1, ge=1, description='Parameter defines the number of adults. Default to 1.')
    children: int = Field(0, ge=0, description='Parameter defines the number of children. Default to 0.')
    ...
    def validate_flight_consistency(self):
        ...
        if flight_type == FlightType.ROUND_TRIP and outbound_date and return_date:
            ...
            if return_d < outbound:
                raise ValueError(f"Return date {return_date} must be after outbound date {outbound_date}")
    ...
    try:
        ...
        if params.type == FlightType.ROUND_TRIP:
            ...
            if len(tokens) < 5 and 'other_flights' in initial_results:
                ...
                for flight in initial_results.get('other_flights', [])[:remaining_slots]:
                    if 'departure_token' in flight:
                        tokens.append(flight['departure_token'])
            ...
            for i, token in enumerate(tokens):
                ...
                try:
                    ...
                    if 'best_flights' in initial_results and i < len(initial_results.get('best_flights', [])):
                        token_result['original_flight_info'] = initial_results['best_flights'][i]
...
# Define flights advisor tools and ReAct agent
flights_advisor_tools = [
    find_flights,
    make_handoff_tool(agent_name="supervisor"),
]
flights_advisor = create_react_agent(
    model=model.bind_tools(flights_advisor_tools, parallel_tool_calls=False, strict=True),
