...
class HotelsInput(BaseModel):
    q: str = Field(description='Location of the hotel (city, area, or specific hotel name)')
    ...
    sort_by: SortingOptions = Field(SortingOptions.RATING_HIGH_TO_LOW,
        ...
        try:
            for hotel_class in classes:
                ...
                if class_int < 1 or class_int > 5:
                    raise ValueError(f"Hotel class must be between 1 and 5, got {hotel_class}")
        except ValueError:
    ...
    def validate_dates_consistency(self):
        ...
        if check_out <= check_in:
            raise ValueError(f"Check-out date {self.check_out_date} must be after check-in date {self.check_in_date}")
